...
void HybridPdfViewer::openPdf(const std::string& filePath) {
    const char* pdf_path = filePath.c_str();
    //if (!pdfDoc) {
    pdfDoc = FPDF_LoadDocument(pdf_path, nullptr);
    //}
}
void HybridPdfViewer::HybridPdfViewer::closePdf() {
    if (pdfDoc) {
       FPDF_CloseDocument(pdfDoc);
    }
}

std::shared_ptr<ArrayBuffer> HybridPdfViewer::getTile(double pageNumber, double row, double column, double displayWidth, double tileSizeD, double scale) {
